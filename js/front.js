...
$('#promoForm').submit(function(e){
        ...
        $.ajax({
            ...
            $('#promoFormSuccess').html("<div class='alert alert-success'>");
            $('#promoFormSuccess > .alert-success').html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>");
            if(data.id){
                //successful adds will have an id attribute on the object
                $('#promoFormSuccess > .alert-success').append("<strong>Awesome! Now check your email to confirm</strong>");
                $('#promoFormSuccess > .alert-success').append('</div>');
                //alert('thanks for signing up');
                $('#promoForm').trigger("reset");
            } else if (data.title === 'Member Exists') {
                //MC wil send back an error object with "Member Exists" as the title
                $('#promoFormSuccess > .alert-success').append("<strong>You already signed up, check your email inbox</strong>");
                $('#promoFormSuccess > .alert-success').append('</div>');
            } else {
                //something went wrong with the API call
                $('#promoFormSuccess > .alert-success').append("<strong>Something went wrong...yeah very cliché we know</strong>");
                $('#promoFormSuccess > .alert-success').append('</div>');
            }
        }).error(function(){
